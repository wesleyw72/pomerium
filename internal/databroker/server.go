...
func (srv *Server) initVersion() {
	dbServerVersion, err := srv.newDB(recordTypeServerVersion)
	if err != nil {
		log.Error().Err(err).Msg("failed to init server version")
		return
	}
	defer dbServerVersion.Close()
...
func (srv *Server) Delete(ctx context.Context, req *databroker.DeleteRequest) (*empty.Empty, error) {
	...
	db, _, err := srv.getDB(req.GetType())
	if err != nil {
		return nil, err
	}
...
func (srv *Server) GetAll(ctx context.Context, req *databroker.GetAllRequest) (*databroker.GetAllResponse, error) {
	...
	db, version, err := srv.getDB(req.GetType())
	if err != nil {
		return nil, err
	}
...
func (srv *Server) doSync(ctx context.Context, serverVersion string, recordVersion *string, db storage.Backend, stream databroker.DataBrokerService_SyncServer) error {
...
func (srv *Server) Sync(req *databroker.SyncRequest, stream databroker.DataBrokerService_SyncServer) error {
	...
	db, serverVersion, err := srv.getDB(req.GetType())
	if err != nil {
		return err
	}
...
func (srv *Server) getDB(recordType string) (db storage.Backend, version string, err error) {
